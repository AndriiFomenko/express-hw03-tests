...
**Складність**: Важке  
**База**: Повністю функціональний Express.js сервер з проміжними обробниками (middleware) (всі тести пройдені)

## Мета завдання

**Взяти існуючий функціональний Express.js сервер** (з проміжними обробниками логування, автентифікації, валідації та REST API) **та розширити його шаблонізаторами PUG і EJS** для створення сучасних HTML сторінок, зберігаючи повну сумісність з існуючими тестами.

## Що вже реалізовано у базовій версії

✅ **Express сервер з проміжними обробниками (middleware)**:

- Логування запитів (`logRequests`)
- Демонстраційна та сувора автентифікація (`demoAuth`, `strictAuth`)
...
- `/users/strict` та `/articles/strict` з обов'язковою автентифікацією
...
app.get('/users', (req, res) => {
  ...
  if (acceptHeader.includes('text/html')) {
    res.render('pug/users', { users: allUsers }) // PUG шаблон
  } else {
...
**Результат**: Існуючий Express сервер з проміжними обробниками (middleware) тепер підтримує як API, так і красиві HTML сторінки з різними шаблонізаторами, демонструючи повне розуміння full-stack веб-розробки.
